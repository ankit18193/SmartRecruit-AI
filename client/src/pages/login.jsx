...
import { LogIn, Loader2, Mail, Lock, ChevronRight, ShieldCheck, ArrowLeft, User, Briefcase } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    if (urlError) {
      if (urlError === 'role_mismatch') {
        toast.error("Role mismatch! Please select the correct role for this email account.");
      } else {
    ...
    if (urlToken && urlRole) {
      ...
      toast.success(`Successfully logged in as ${urlRole}! `);
      window.history.replaceState({}, document.title, window.location.pathname);
      
      const userRole = String(urlRole).toLowerCase();
      
      window.location.replace(userRole === 'recruiter' ? '/admin' : '/dashboard');
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (actualRole !== selectedRole) {
        toast.error(`Role mismatch! This email is registered as a ${actualRole}.`);
        setLoading(false);
        return;
      }
      ...
      window.location.href = actualRole === 'recruiter' ? '/admin' : '/dashboard';
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-[440px] z-10 animate-in fade-in zoom-in duration-500">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">Welcome to SmartRecruit</h1>
          <p className="text-slate-500 font-medium mt-3">
            {step === 1 ? "How would you like to continue?" : `Log in to your ${selectedRole} account`}
          </p>
        </div>
            ...
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    ...
                    <input
                      ...
                      className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:bg-white focus:ring-4 focus:ring-blue-600/5 focus:border-blue-600 transition-all font-medium text-slate-700"
                ...
                <button
                  ...
                  className="w-full bg-slate-900 text-white py-4 rounded-2xl font-bold shadow-xl shadow-slate-200 hover:bg-slate-800 active:scale-[0.98] transition-all flex items-center justify-center gap-3 disabled:opacity-70 group mt-2"
                >
                  {loading ? <Loader2 className="animate-spin" size={22} /> : (
                    <><span>Sign In as {selectedRole.charAt(0).toUpperCase() + selectedRole.slice(1)}</span> <ChevronRight size={18} className="group-hover:translate-x-1 transition-transform" /></>
                  )}
                </button>
              </form>
              ...
              <div className="relative mt-8 mb-6 text-center">
                ...
                <span className="relative px-4 text-slate-400 text-[10px] font-black uppercase tracking-[0.2em] bg-white/0">
                  Or continue with
                </span>
              </div>
