...
const CandidateDashboard = () => {
  ...
  const activeResumeId = useRef(null);
  ...
  useEffect(() => {
    const loadDashboard = async () => {
      try {
        ...
        setAppliedJobIds(
          new Set(apps.map((app) => app.jobId?._id || app.jobId)),
        );
        ...
        const interviews = apps.filter(
          (a) =>
            [
              ...
              "Selected",
              "Accepted",
            ].includes(a.status) || a.interviewScore,
        ).length;
    ...
    const loadResumeJobs = async () => {
      ...
      try {
        const res = await API.get(
          `/resume/recommend-jobs/${selectedResume._id}`,
        );
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="flex flex-1">
        <main className="flex-1 p-6 overflow-y-auto">
          ...
          {selectedResume ? (
            ...
            <div className="bg-white border border-dashed border-gray-300 rounded-2xl p-10 text-center mb-8">
              ...
              <p className="text-sm text-gray-500 mt-1">
                Upload your resume in the right panel to see jobs perfectly
          ...
          <RecentList apps={recentApplications} user={userData} />
        </main>
            ...
            >
              <Bell
                ...
                className={`text-gray-700 transition ${
                  unreadCount > 0 ? "animate-bounce text-blue-600" : ""
                }`}
              />
            ...
            {showProfileCard && (
              <div className="absolute right-0 top-full mt-2 w-56 bg-white border border-gray-100 rounded-2xl shadow-xl py-2 z-50 animate-[fadeIn_0.2s_ease]">
                <div className="px-5 py-4 border-b border-gray-50 mb-2 flex flex-col items-center text-center">
                  ...
                  <p className="text-sm font-bold text-gray-800 w-full truncate">
...
const AiPanel = ({ resume, uploading, onUpload }) => {
  ...
  const currentData =
    open === "s"
      ? safeInsights.strengths
      : open === "m"
        ? safeInsights.missingSkills
        : open === "i"
          ? safeInsights.suggestions
          : [];
  ...
  return (
    <div className="w-[340px] bg-white border-l p-5 h-screen sticky top-0 overflow-y-auto shadow-lg pb-24 z-40">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4 mb-5 shadow-sm">
        <div className="flex justify-between items-center mb-3">
          ...
          <span
            className={`text-[10px] font-bold px-2 py-1 rounded-md uppercase tracking-wide ${
              marketLevel === "top-tier"
                ? "bg-purple-100 text-purple-700"
                : marketLevel === "job-ready"
                  ? "bg-green-100 text-green-700"
                  : marketLevel === "intermediate"
        ...
        <div className="relative w-full flex flex-col items-center justify-center">
          ...
          <div className="w-full mt-2 bg-gray-200 rounded-full h-2 overflow-hidden">
            <div
              className="h-2 bg-gradient-to-r from-blue-500 to-indigo-500 transition-all duration-700"
              style={{ width: `${aiScore}%` }}
            ></div>
          </div>
      ...
      <div className="mb-4">
        <label
          className={`block w-full border-2 border-dashed text-center py-4 rounded-xl transition-all duration-200 ${
            uploading
              ? "bg-gray-100 border-gray-200 cursor-not-allowed opacity-60"
              : "border-blue-300 bg-blue-50 hover:bg-blue-100 hover:shadow-md cursor-pointer"
          }`}
        >
      ...
      {resume ? (
        ...
        <div className="text-center text-xs text-gray-400 mb-5 border border-dashed rounded-xl p-4">
      ...
      {resume && (
        <div className="space-y-3">
          <div
            onClick={() => setOpen("s")}
            className="p-3 bg-green-50 border border-green-100 rounded-xl cursor-pointer hover:shadow-md transition flex justify-between items-center group"
          >
            <div className="bg-green-50 border border-green-200 rounded-xl p-3 shadow-sm hover:shadow-md transition">
              <h3 className="text-sm font-bold text-green-700 mb-2">
                ✅ Strengths
              </h3>
              <ul className="text-xs text-gray-700 space-y-1">
                {Object.values(resume?.insights?.strengths || {}).map(
                  (item, i) => (
            ...
            <span className="text-xs bg-green-200 text-green-800 px-2 py-0.5 rounded-full">
              {safeInsights.strengths.length}
            </span>
          </div>

          <div
            onClick={() => setOpen("m")}
            className="p-3 bg-red-50 border border-red-100 rounded-xl cursor-pointer hover:shadow-md transition flex justify-between items-center group"
          >
            <div className="bg-red-50 border border-red-200 rounded-xl p-3 shadow-sm hover:shadow-md transition">
              <h3 className="text-sm font-bold text-red-700 mb-2">
                ❌ Missing Skills
              </h3>
              <ul className="text-xs text-gray-700 space-y-1">
                {Object.values(
                  resume?.insights?.missingSkills || {},
                ).map((item, i) => (
                  <li key={i}>• {item.label}</li>
                ))}
              </ul>
            ...
            <span className="text-xs bg-red-200 text-red-800 px-2 py-0.5 rounded-full">
              {safeInsights.missingSkills.length}
            </span>
          </div>

          <div
            onClick={() => setOpen("i")}
            className="p-3 bg-blue-50 border border-blue-100 rounded-xl cursor-pointer hover:shadow-md transition flex justify-between items-center group"
          >
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 shadow-sm hover:shadow-md transition">
              <h3 className="text-sm font-bold text-blue-700 mb-2">
                💡 Suggestions
              </h3>
              <ul className="text-xs text-gray-700 space-y-1">
                {Object.values(resume?.insights?.suggestions || {}).map(
                  (item, i) => (
                    <li key={i}>• {item.label}</li>
                  ),
                )}
            ...
            <span className="text-xs bg-blue-200 text-blue-800 px-2 py-0.5 rounded-full">
              {safeInsights.suggestions.length}
            </span>
          </div>
          ...
          {careerStrategy?.shortTerm && (
            <div
              onClick={() => setOpen("c")}
              className="p-3 bg-purple-50 border border-purple-100 rounded-xl cursor-pointer hover:shadow-md transition flex justify-between items-center group"
            >
              <p className="text-sm font-semibold text-purple-700 group-hover:translate-x-1 transition-transform">
                Career Strategy
              </p>
      ...
      {open && (
        ...
        >
          <div
            className="bg-white w-[550px] max-w-[95%] max-h-[85vh] overflow-y-auto pr-2 rounded-3xl p-8 shadow-2xl relative animate-[scaleIn_0.2s_ease]"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => setOpen(null)}
              className="absolute top-5 right-6 text-gray-400 hover:text-gray-800 bg-gray-100 rounded-full w-8 h-8 flex items-center justify-center transition-colors"
            >
              ✕
            </button>
            ...
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100">
              <span
                className={`text-3xl ${open === "s" ? "text-green-500" : open === "m" ? "text-red-500" : open === "i" ? "text-blue-500" : "text-purple-500"}`}
              >
                {open === "s"
                  ? "💪"
                  : open === "m"
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1 font-medium">
                  {open === "c"
                    ? "Step-by-step strategy to level up"
                    : "Deep analysis based on your latest resume"}
                </p>
              </div>
            </div>
                        ...
                        {items.map((item, i) => (
                          ...
                          >
                            <div className="flex justify-between items-start mb-3">
                              ...
                              <span
                                className={`text-[10px] font-bold px-2.5 py-1 rounded-md uppercase tracking-widest ml-2 shrink-0 ${
                                  item.impact === "high" ||
                                  item.priority === "high"
                            ...
                            <p className="text-sm text-gray-700 leading-relaxed font-medium">
                              {item.reasoning ||
                                "Context not provided for this insight."}
                            </p>
                          </div>
                        ))}
                      </div>
...
const JobGrid = ({
  ...
  useEffect(() => {
    if (user?.savedJobs) {
      const savedIds = user.savedJobs.map((job) =>
  ...
  const handleSave = async (jobId) => {
    ...
    try {
      ...
      setSavedJobs((prev) =>
        isCurrentlySaved ? [...prev, jobId] : prev.filter((id) => id !== jobId),
      );
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-400 text-xs">
          Try improving your resume or adding more skills{" "}
        </p>
      </div>
    );
  }
  ...
  return (
    <>
            ...
            return (
              ...
              >
                  ...
                  {matchedSkills.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                      {topSkills.map((skill, index) => (
                        <span
                          key={index}
                          className="bg-green-100 text-green-700 text-[10px] px-2 py-1 rounded-full"
                        >
                          ✔ {skill}
                        </span>
                      ))}
                    ...
                    <p className="text-[10px] text-gray-400">
                      Few exact skills matched — check details
                    </p>
                  )}
                </div>
                ...
                <div className="mb-3 space-y-1">
                  <p
                    className={`text-[10px] font-bold ${getColor(skillScore)}`}
                  >
                ...
                <div className="flex flex-wrap justify-between items-center gap-2 border-t border-gray-100 pt-4 mt-auto">
                  ...
                  <button
                    ...
                    className={`text-xs transition hover:scale-105 ${
                      isSaved
                        ? "text-yellow-500 font-bold bg-yellow-50 px-3 py-1.5 rounded-md"
                        : "text-gray-400 hover:text-gray-600 px-3 py-1.5"
                    }`}
                  >
                    {isSaved ? "Saved ⭐" : "Save ⭐"}
                  </button>
                ...
                <div className="flex justify-between w-full mt-3">
                  <button
                    ...
                    className="text-[10px] font-semibold text-red-400 hover:text-red-600 transition hover:scale-105"
                  >
                    Not Interested ❌
                  </button>
                  <button
      ...
      {selectedJob && (
        ...
        >
          <div
            className="bg-white w-[320px] rounded-xl p-5 shadow-xl relative animate-[scaleIn_0.2s_ease]"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => setSelectedJob(null)}
              ...
              return (
                <div className="text-xs text-gray-600 space-y-2 leading-relaxed">
                  <p className="text-green-600 font-bold">
                    You match {exp.matchedCount}/{exp.total} key skills
                  </p>
                  {exp.missingSkills.length > 0 && (
                    <p className="text-red-500 font-medium">
                      {" "}
                      Missing: {exp.missingSkills.join(", ")}
                    </p>
                  )}
                  <p className="text-blue-500 font-medium">
                    Improve: Add more relevant projects or skills
                  </p>
                </div>
              );
            })()}
...
const SmartInterviewButton = ({ jobId }) => {
  ...
  const isCompleted =
    interviewData?.status === "Completed" || interviewData?.score !== undefined;
...
const StatCard = ({ title, value, icon, color = "blue" }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`w-10 h-10 flex items-center justify-center rounded-lg ${colorMap[color]}`}
      >
        {icon}
      </div>
    </div>
...
const SectionTitle = ({ title }) => (
  <h2 className="text-lg font-semibold text-gray-800 mb-4 mt-6">{title}</h2>
);
